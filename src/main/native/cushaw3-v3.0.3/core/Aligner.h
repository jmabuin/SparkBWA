...
public:
	...
	vector<CigarAlign*> galignPath(uint8_t* s1, uint8_t* s2,
...
private:
	...
	CigarAlign* align2cigar(uint8_t* s1, uint8_t* s2, int32_t s1Length,
			int32_t s2Length, uint8_t* tbtble, int32_t bandBufferSize,
			int32_t cibest, int32_t cjbest, int32_t low, int32_t alignScore,
			bool rev);

	CigarAlign* align2cigar(uint8_t* s1, uint8_t* s2, int32_t s1Length,
			int32_t s2Length, uint8_t* tbtble, int32_t cibest, int32_t cjbest,
			int32_t alignScore);
	...
	inline uint8_t getAlignMove(uint8_t* pdata, int32_t coord)
	{
		return (pdata[coord >> 2] >> ((coord & 3) << 1)) & 3;
	}

	inline void setAlignMove(uint8_t* pdata, int32_t coord, uint8_t dir)
	{
		uint8_t nbyte = coord >> 2;
		uint8_t shift = (coord & 3) << 1;
		uint8_t data = pdata[nbyte];
